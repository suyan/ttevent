	...
	setUserToken : function(qq_name, qq_uid){
	    if(sessionStorage.getItem('cootoken')){
    	    ...
    	    $.getJSON(website + '/user/getusercootoken?callback=?',{'qq_name':qq_name, 'qq_uid':qq_uid}, function(data) {
                ...
                //再开一个记录，记录用户是否一直保持在线
                EventView.utils.isLogin.prototype.singnal = 'True'          
                //再show一下page，防止异步json数据没有请求完毕，listview不刷新
                //这个太hack了，暂时没有其他招                       
                EventView.controllers.user.pageshow()          
    	    });
    	}
	},
        ...
        if(obj){
            ...
            for(var i in obj){
                ...
                switch(typeof val){
                    case ("object"):
                        if(val && val[0]){
                            output += "'" + i + "':" + EventView.utils.array2String(val) + ",";
                        }else{
                            output += "'" + i + "':" + EventView.utils.object2String(val) + ",";
                        }
                        break;
                    case ("string"):
        ...
        if(array){
            ...
            for(var i in array){
                ...
                switch(typeof val){
                    case ("object"):
                        if(val[0]){
                            output += EventView.utils.array2String(val) + ",";
                        }else{
                            output += EventView.utils.object2String(val) + ",";
                        }
                        break;
                    case ("string"):
...
// 业务控制中心，需应用实现
EventView.controllers = {}
// 事件注册
EventView.run = function(pages){
	var pages = pages,
	count = pages.length;
	for(var i=0;i<count;i++){
		...
		for(var j=0; j <e_array.length;j++){
			...
			if(e.replace(/(^\s*)|(\s*$)/g,'').length == 0)
				continue;
			$("#"+id).bind(e, EventView.controllers[id][e]);
		}
	}
